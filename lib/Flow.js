...
export default function flowClassBuilder(queue, mongoCon, options) {
    ...
    class Flow {
        ...
        constructor(job) {
            ...
            this.flowId = job.data.flowId;
            this.stepsTaken = job.data.stepsTaken;
            this.substepsTaken = job.data.substepsTaken;
             ...
             * unpackPromises function (check .done() for more)
             * @type {Array}
             */
            this.jobHandlers = [];
                        ...
                        _this.FlowModel.findById(_this.flowId)
                            .then((flowDoc, err) => {
                                    ...
                                    this.FlowModel.create(
                                        ...
                                        .then((flowDoc, err) => {
                                            if (err) {
                                                ...
                                                Logger.info(`[${_this.flowId}] New Flow created. Flow.init() complete.`);
                                                resolve(_this);
                                            }
                                        })
                                    ;
                                ...
                                // Found the _id in Mongo, we are restarting a failed Flow
                                    ...
                                    this.substepsTaken = flowDoc.substepsTaken;
                                    ...
                                    reject(new Error(`[${_this.flowId}] Something went very very wrong when init()ing Flow...`));
                                }
                            })
                        ;
                    }
                    else {
                        reject(new Error(`[${_this.flowId}] flowId passed to Flow.init() is not a valid ObjectId.`));
                    }
        ...
        handleJob(step, substep, asyncJobReturner, restartJob = (()=> Logger.debug(`[${this.flowId}] No restartJob() passed.`))) {
            ...
            Logger.debug(`[${this.flowId}] Handling step ${step}, substep ${substep}`);
            ...
            return new Promise((handleJobResolve, handleJobReject) => {
                if (step < 1) {
                ...
                else if ((step === (_this.stepsTaken + 1) && substep > _this.substepsTaken) || (step > _this.stepsTaken + 1)) {
                    ...
                    if (promised[ stepStr ]) {
                        ...
                        Logger.debug(`[${this.flowId}] Added job for step: ${step}`);
                        handleJobResolve();
                    }
                    else {
                        Logger.debug(`[${this.flowId}] Added job for step: ${step}`);
        ...
        completeJob(job) {
            ...
            return new Promise((resolve, reject) => {
                if (!job) {
                    ...
                    this.FlowModel.findByIdAndUpdate(_this.flowId, { substepsTaken: _this.substepsTaken + 1 }, { new: true })
                        .then((flowDoc, err) => {
                            if (err) {
                                ...
                                this.JobModel.findOneAndUpdate({_id: job.data.uuid}, { complete: true }, { new: true })
                                    .then((jobDoc, err) => {
                                        if (err) {
                                            reject(err);
                                        }
        ...
        job(step, jobName, jobData) {
            ...
            //let positions = jobData.positions;
            //
            //// TODO remove  && false, check this path works
            //if (positions.length !== 1 && false) {
            //    positions.forEach((position) => {
            //        let modJobData = jobData;
            //        modJobData.positions = [ position ];
            //        _this.job(step, jobName, modJobData);
            //    });
            //}
            if (true) {
                let substep;
                let substepsTaken = _this.substeps[ step ];

                // If we already have substeps at this step, increase substeps by 1 and set substep to the result
                if (substepsTaken) {
                    this.substeps[ step ] += 1;
