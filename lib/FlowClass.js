...
exports['default'] = flowClassBuilder;
...
var setPath = require('./util/setPath');
...
function flowClassBuilder(queue, mongoCon, FloughInstance, startFlow) {
    ...
    var Flow = (function () {
        ...
        function Flow(job) {
            ...
            _this.parentUUID = job.data._parentFlowId;
        ...
        _createClass(Flow, [{
            ...
            value: function start() {
                var promiseArray = arguments.length <= 0 || arguments[0] === undefined ? [] : arguments[0];
                ...
                _this.promised['0'].push(new Promise(function (resolve, reject) {
                    ...
                    try {
                        ...
                        if (_this.FlowModel.isObjectId(_this.flowId)) {
                            ...
                            _this.FlowModel.findById(_this.flowId).then(function (flowDoc, err) {
            ...
            key: 'job',
            value: function job(step, jobType) {
                var jobData = arguments.length <= 2 || arguments[2] === undefined ? {} : arguments[2];

                var _this = this;
                Promise.all(_this.promised['0']).then(function (promised) {
                    var substep = undefined;
                        ...
                        return _this.handleChild(step, substep, function (currentRelatedJobs) {
                            return new Promise(function (jobResolve, jobReject) {
                                try {
                                    ...
                                    var jobUUID = _.get(currentRelatedJobs, step + '.' + substep + '.data._uuid', null);
                                    ...
                                    try {
                                        for (var _iterator = Object.keys(finalJobData._relatedJobs)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                                            var key = _step.value;
                                    ...
                                    FloughInstance.startJob(jobType, finalJobData).then(function (job) {

                                        // When job is enqueued into Kue, relate the job to this flow.
                                        var relateJobPromise = undefined;
                                        var updateJobInMongoPromise = undefined;
                                        job.on('enqueue', function () {
                                            relateJobPromise = _this.updateAncestors(job, step, substep);
                                            updateJobInMongoPromise = _this.updateJobId(job, step, substep);
                                        });

                                        // When job is complete, resolve with job and result.
                                        job.on('complete', function (result) {
                                            Promise.join(relateJobPromise, updateJobInMongoPromise).then(function () {
                                                _this.jobLogger('Job logic complete.', job.data._uuid, job.id);
                                                jobResolve([job, result ? result : null]);
                                            })['catch'](function (err) {
                                                return jobReject(err);
                                            });
                                        });
            ...
            key:   'updateJobId',
            value: function updateJob(job, step, substep) {
                var _this = this;
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    var numTries = 0;
                    var maxTries = 4;
                    var clearTheInterval = function clearTheInterval() {
                        return clearInterval(updateInterval);
                    };

                    var updateTheJob = function updateTheJob() {
                        numTries += 1;
                        _this.JobModel.findOneAndUpdate({ 'data._uuid': job.data._uuid }, { jobId: job.id }, /*{new: true}, */function (err, jobDoc) {
                            if (err && maxTries > 4) {
                                ...
                                clearTheInterval();
                                var errorMsg = 'Error updating job in MongoDB with new job id: Could not find job UUID of ' + job.data._uuid + ' in MongoDB';
            ...
            key: 'flow',
            value: function flow(step, flowType) {
                var jobData = arguments.length <= 2 || arguments[2] === undefined ? {} : arguments[2];
                        ...
                        return _this.handleChild(step, substep, function (currentRelatedJobs) {
                            return new Promise(function (jobResolve, jobReject) {
                                try {
                                    ...
                                    try {
                                        for (var _iterator2 = Object.keys(finalJobData._relatedJobs)[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                                    ...
                                    startFlow(flowType, finalJobData, true).then(function (flowJob) {

                                        // When job is enqueued into Kue, relate the job to this flow.
                                        var relateJobPromise = undefined;
                                        flowJob.on('enqueue', function () {

                                            // TODO? Maybe have to also update flow's jobId lke in job function
                                            relateJobPromise = _this.updateAncestors(flowJob, step, substep);
                                        });

                                        // When job is complete, resolve with job and result.
                                        flowJob.on('complete', function (result) {
                                            relateJobPromise.then(function () {
            ...
            key: 'execF',
            value: function execF(step, promReturningFunc) {
                var _this = this;

                if (_this.stepsTaken < step) {

                    var promFunc = function promFunc() {

                        var relatedJobs = _this.relatedJobs;
            ...
            key:   'handleChild',
            value: function handleJob(step, substep, jobRunner) {
                var _this2 = this;

                var restartJob = arguments.length <= 3 || arguments[3] === undefined ? function () {
                    return Logger.debug(_this2.loggerPrefix + ' No restartJob() passed.');
                } : arguments[3];
                ...
                return new Promise(function (handleJobResolve, handleJobReject) {
                    if (step < 1) {
                     ...
                     */
                             ...
                             * @returns {bluebird|exports|module.exports}
                             */
                            var runJob = function runJob(currentRelatedJobs) {
                                return new Promise(function (resolve, reject) {
                                    // Run the job...
                                    jobRunner(currentRelatedJobs)

                                    // Complete the job...
                                    .spread(function (job, result) {
            ...
            key:   'updateAncestors',
            value: function relateJob(job, step, substep) {
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    _this.FlowModel.findOneAndUpdate({ _id: _this.flowId }, {
                        $set: _defineProperty({}, 'relatedJobs.' + step + '.' + substep, {
                            data: job.data,
                            result: null
                        })
                    }, { 'new': true }, function (err, flowDoc) {
                        if (err) {
                        ...
                        // If this job is part of a helper flow, update parent flows relatedJobs with this info
                                ...
                                if (_this.parentUUID) {
                                    ...
                                    _this.FlowModel.findOneAndUpdate({ _id: _this.parentUUID }, {
                                        $set: _defineProperty({}, 'relatedJobs.' + _this.data._step + '.' + _this.data._substep + '.data._relatedJobs', flowDoc.relatedJobs)
                                    }, { 'new': true, upsert: true }, function (err, parentFlowDoc) {
                                        if (err) {
            ...
            value: function end() {
                ...
                function cleanupRelatedJobs() {
                    ...
                    return new Promise(function (resolve, reject) {
                        ...
                        _this.FlowModel.findById(_this.flowId, function (err, flowDoc) {
                            if (err) {
                                reject(err);
                            } else if (flowDoc) {
                                (function () {
                                    ...
                                    _this.relatedJobs = _(_this.relatedJobs).pick(_.range(1, _this.stepsTaken + 2)).mapValues(function (substepsObj, step, obj) {
                                        var stepNum = parseInt(step, 10);

                                        if (stepNum < _this.stepsTaken) {
                                            return substepsObj;
                                        } else {
                                            _.forOwn(substepsObj, function (flowData, substep) {
                                                if (!_.get(flowData, '.data._uuid', false)) {
                                                    subFlows.push({ step: step, substep: substep, flowId: flowData.data._flowId });
                                                }
                                            });
                                    ...
                                    var attachFlowProgress = function attachFlowProgress(_ref) {
                                        var step = _ref.step;
                                        var substep = _ref.substep;
                                        var flowId = _ref.flowId;

                                        return new Promise(function (resolve, reject) {
                                            _this.FlowModel.findOne(flowId, function (err, doc) {
                                                if (err) {
                                                    Logger.error(err.stack);
                                                    reject(err);
                                                } else {
                                                    ...
                                                    resolve({ step: step, substep: substep, doc: doc });
                                                }
                                            });
                                        });
                                    };

                                    Promise.all(subFlows.map(attachFlowProgress)).then(function (docInfos) {
                                        flowDoc.relatedJobs = _this.relatedJobs;
                                        flowDoc.save(function (err) {
                                            if (err) {
                                                reject(err);
                                            } else {
                                                resolve();
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    .then(setStepsTakenToOne).then(function () {

                        // 2.
                        var jobHandlerPromises = _this.flowHandlers.map(function (promiseReturner) {
                            return promiseReturner();
                        });

                        // Find largest step number attached to _this.promised
                        var lastStep = Math.max.apply(Math, _toConsumableArray(Object.keys(_this.promised).map(function (string) {
                            return parseInt(string, 10);
                        })));

                        // 3.
                        Promise.all(jobHandlerPromises).then(function () {
                        ...
                        function unpackPromises(_x5) {
                            ...
                            _function: while (_again) {
                                ...
                                var promiseReturners = _this.promised[stepKey];

                                // Pass current related jobs to jobs/flows
                                var currentRelatedJobs = _this.relatedJobs;
                                ...
                                if (promiseReturners) {
                                    ...
                                    var promiseList = promiseReturners.map(function (promiseReturner) {
                                    ...
                                    _x5 = step + 1;
                                    _again = true;
                                    stepKey = promiseReturners = currentRelatedJobs = promiseList = undefined;
                                    continue _function;
                                } else if (step > lastStep) {
                                    resolve(_this);
                                }
            ...
            key: 'setFlowResult',
            value: function setFlowResult(result) {
                var _this = this;
                ...
                return new Promise(function (resolve, reject) {
                    _this.FlowModel.findByIdAndUpdate(_this.flowId, {
                        completed: true,
                        result: result
                    }, { 'new': true }).then(function (flowDoc, err) {
                        if (err) {
            ...
            key:   'completeChild',
            value: function completeJob(job, jobResult) {
                var _this = this;
                return new Promise(function (resolve, reject) {
                    if (job) {
                        // Create field to update
                        var relatedJobResultField = 'relatedJobs.' + job.data._step + '.' + job.data._substep + '.result';
                        ...
                        _this.FlowModel.findByIdAndUpdate(_this.flowId, {
                            ...
                            $set: _defineProperty({}, relatedJobResultField, jobResult)
                        }, { 'new': true }).then(function (flowDoc, err) {
                            if (err) {
                                ...
                                // Remove job from activeChildren
                                _this.activeChildren = _.remove(_this.activeChildren, function (activeJob) {
            ...
            key: 'cancel',
            value: function cancel(cancellationData) {
                var _this = this;

                return Promise.all(_this.promised['0']).then(function () {
                    ...
                    return new Promise(function (resolve, reject) {
                        ...
                        _this.FlowModel.findByIdAndUpdate(_this.flowId, { isCancelled: true }, { 'new': true }, function (err, flowDoc) {
                            if (err) {
                                ...
                                var errorMsg = 'FlowId of ' + _this.flowId + ' is not in MongoDB and could not be set to cancelled.';
                                Logger.error(errorMsg);
                                cancelFlowJob();
                                reject(errorMsg);
                            } else {
