...
export default function flowClassBuilder(queue, mongoCon, FloughInstance, startFlow) {
    ...
    class Flow {
            ...
            _this.promised['0'].push(new Promise((resolve, reject) => {
                ...
                try {
                    ...
                    if (_this.FlowModel.isObjectId(_this.flowId)) {
                        ...
                        _this.FlowModel.findById(_this.flowId).then((flowDoc, err) => {
        ...
        job(step, jobType, jobData = {}) {
            ...
            let _this = this;
            Promise.all(_this.promised['0']).then(promised => {
                let substep;
                    ...
                    return _this.handleJob(step, substep, currentRelatedJobs => {
                        return new Promise((jobResolve, jobReject) => {
                            try {
                                ...
                                FloughInstance.startJob(jobType, finalJobData).then(job => {
                                    ...
                                    job.on('complete', result => {
                                        Promise.join(relateJobPromise, updateJobInMongoPromise).then(() => {
                    ...
                    return _this.handleJob(step, substep, currentRelatedJobs => {
                        return new Promise((jobResolve, jobReject) => {
                            try {
                                ...
                                startFlow(flowType, finalJobData, true).then(flowJob => {
         ...
         */
                ...
                return new Promise((resolve, reject) => {
                    ...
                    _this.FlowModel.findById(_this.flowId, (err, flowDoc) => {
                        if (err) {
                            ...
                            _this.relatedJobs = _(_this.relatedJobs).pick(_.range(1, _this.stepsTaken + 2)).mapValues((substepsObj, step, obj) => {
                            ...
                            Promise.all(subFlows.map(attachFlowProgress)).then(docInfos => {
            ...
            return new Promise((resolve, reject) => {
                ...
                .then(setStepsTakenToOne).then(() => {
                    ...
                    // 3.
                    Promise.all(jobHandlerPromises).then(() => {
