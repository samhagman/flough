...
export default function flowAPIBuilder(queue, mongoCon, FloughInstance) {
    ...
    function registerFlow(flowType, flowOptions, flowFunc, dynamicPropFunc) {
        ...
        queue.process(`flow:${ flowType }`, jobProcessingConcurrency, (job, done) => {
            ...
            if (o.devMode) {
                flowWrapper(job, flow).then(result => flow.setFlowResult(result)).tap(result => Logger.info(`[${ job.type }][${ flow.flowId }][${ job.id }] COMPLETE - RESULT: ${ JSON.stringify(result, null, 2) }`)).then(result => done(null, result)).catch(err => {
            ...
            // In production mode catch errors to prevent crashing
                    ...
                    flowWrapper(job, flow).then(result => flow.setFlowResult(result)).then(result => done(null, result)).catch(err => done(err));
                }
        });
    }
